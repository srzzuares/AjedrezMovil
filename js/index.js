        ...
        for(let i=0;i<pieces.length;i++){
            if(pieces[i].color != currentPlayer.color){
                ...
                if(pieces[i].isValidMove(getSquare(currentPlayer.king.x, currentPlayer.king.y),n-1).valid){
                    result.valid = false;
                    console.log(pieces[i]);
                    console.log("prevents king from moving Esto como?? ");
...
let squareClicked = function(e){
	...
	if(selectedSquare === null){
		if(square.piece === null){
			showError("No hay ninguna pieza aquí");
		}else if(square.piece.color != currentPlayer.color){
			showError("Elige una pieza de tu color");
		}else{
	...
	if(moveResult.valid)
    {
        ...
        if(kingExposed(currentPlayer.king)){
            //if(!(piece instanceof King)){
                console.log("exposed");
                showError("Movimiento no permitido Humano!");
                end.unsetPiece();
                piece.x = start.x;
                piece.y = start.y;
		...
		showError("Movimiento no permitido.");
        piece.x = start.x;
        piece.y = start.y;
...
let nextTurn = function(){
	...
	if(currentPlayer.color == "white"){
		currentPlayer = black;
        document.getElementById("turnInfo").innerHTML = "Turno del Jugador de color: <b>Negro</b>";
	}else{
		currentPlayer = white;
        document.getElementById("turnInfo").innerHTML = "Turno del Jugador de color: <b>Blanco</b>";
	}
}
